        ...
        if (intervention_id) {
            async function fetchData() {
                const res = await axios.get(
                    BaseUrl + `/api/tier/${intervention_id}`
                );
                const artWorkData = res.data.tiers;
                setTiers(artWorkData);
            }
            fetchData();
        }
    }, []);
